...
class BybitClient:
    ...
    async def create_stop_loss_order(
        ...
        try:
            params = params or {}
            params.update({
                # Дублируем trigger-поля для совместимости разных версий/режимов Bybit в ccxt.
    ...
    async def get_total_unrealized_pnl(self) -> Decimal:
        """
        Возвращает суммарный unrealized PnL по всем открытым позициям.
        """
        try:
            ...
            for position in positions:
                ...
                raw_unrealized = (
                    ...
                    or (position.get('info') or {}).get('unrealisedPnl')
                    or (position.get('info') or {}).get('unrealizedPnl')
                    or 0
                )
                try:
                    total_unrealized += Decimal(str(raw_unrealized))
                except Exception:
                    continue

            return total_unrealized.quantize(Decimal('0.01'))
        except Exception as e:
            raise Exception(f"Ошибка получения unrealized PnL: {str(e)}")
