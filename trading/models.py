...
class UserTradingSettings(models.Model):
    ...
    stop_loss_atr_multiplier = models.DecimalField(
        ...
        decimal_places=1, 
        default=Decimal('1.5'),
        verbose_name='Множитель ATR для стоп-лосса'
    )
    take_profit_atr_multiplier = models.DecimalField(
        max_digits=3, 
        decimal_places=1, 
        default=Decimal('2.5'),
        verbose_name='Множитель ATR для тейк-профита'
    )
    ...
    def __str__(self):
        return f"Настройки {self.user.username} | Размер: ${self.base_order_size} | Плечо: {self.leverage}x | Риск: {self.risk_per_trade}%"
